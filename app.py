...
def calculate_percent():
    st.write(st.session_state.correct / st.session_state.total + "%")
...
if st.selectbox('Tools', ['Randomizing Tool', 'Drawing Tool']) == 'Randomizing Tool':
    ...
    if uploaded_file is not None:
        ...
        if "correct" not in st.session_state:
            ...
            st.session_state.correctness = 0

        if "incorrect" not in st.session_state:   
            st.session_state.incorrect = 0
            st.session_state.incorrectness = 0

        if "total" not in st.session_state:
            st.session_state.total = 0

        st.button("Right prediction", on_click=increment_correct)
        st.button("Wrong prediction", on_click=increment_incorrect)
        if st.button("Calculate correct prediction in percent:"):
            calculate_percent() 

        # st.write("Correct:", st.session_state.correct, st.session_state.correct / st.session_state.total if st.session_state.correct > 0 else "No prediction", "%")
...
# st.session_state.correct / st.session_state.total if st.session_state.correct > 0 else "No prediction" 
